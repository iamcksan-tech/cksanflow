...
function App() {
  ...
  const [cardForm, setCardForm] = useState({ name: '', limit: '', available: '', balance: '', paymentDate: '26th', thisCyclePayment: '', nextCyclePayment: '' });
  const [showGoalForm, setShowGoalForm] = useState(false);
  const [editingGoal, setEditingGoal] = useState(null);
  const [goalForm, setGoalForm] = useState({ name: '', target: '', color: '#14b8a6', priority: 'medium' });
  ...
  const handleOpenCardForm = (card = null) => {
    if (card) {
      setEditingCard(card);
      setCardForm({ name: card.name, limit: card.limit.toString(), available: card.available.toString(), balance: card.balance.toString(), paymentDate: card.paymentDate, thisCyclePayment: card.thisCyclePayment.toString(), nextCyclePayment: card.nextCyclePayment.toString() });
    } else {
      setEditingCard(null);
  ...
  const handleSaveCard = () => {
    ...
    if (editingCard) {
      setCreditCards(creditCards.map(card => card.id === editingCard.id ? { ...card, name: cardForm.name, limit, available: finalAvailable, balance: finalBalance, paymentDate: cardForm.paymentDate, closingDate, thisCyclePayment: parseFloat(cardForm.thisCyclePayment) || 0, nextCyclePayment: parseFloat(cardForm.nextCyclePayment) || 0 } : card));
      alert('✅ Card updated!');
    } else {
      setCreditCards([...creditCards, { id: Date.now(), name: cardForm.name, limit, available: finalAvailable, balance: finalBalance, paymentDate: cardForm.paymentDate, closingDate, thisCyclePayment: parseFloat(cardForm.thisCyclePayment) || 0, nextCyclePayment: parseFloat(cardForm.nextCyclePayment) || 0 }]);
      alert('✅ Card added!');
    }
    handleCloseCardForm();
  };
    ...
    <div className="App" style={{ minHeight: '100vh', background: darkMode ? '#0f172a' : '#f1f5f9', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {['overview', 'cards', 'goals', 'expenses', 'income'].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '12px 20px', background: activeTab === tab ? '#14b8a6' : (darkMode ? '#334155' : '#f1f5f9'), color: activeTab === tab ? 'white' : (darkMode ? '#94a3b8' : '#647480'), border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: activeTab === tab ? '700' : '600', fontSize: '14px', textTransform: 'capitalize' }}>
                {tab === 'overview' && '📊 '}{tab === 'cards' && '💳 '}{tab === 'goals' && '🎯 '}{tab === 'expenses' && '📋 '}{tab === 'income' && '💵 '}{tab}
              </button>
            ))}
          </div>
      ...
      <main style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
        {activeTab === 'overview' && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px', marginBottom: '32px' }}>
              <div onClick={() => setActiveTab('income')} style={{ background: 'linear-gradient(135deg, #059669 0%, #10b981 100%)', borderRadius: '20px', padding: '28px', color: 'white', cursor: 'pointer', boxShadow: '0 8px 24px rgba(16,185,129,0.3)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                  <div><p style={{ margin: '0 0 4px 0', fontSize: '14px', fontWeight: '600', opacity: 0.9 }}>💵 Cash Balance</p><p style={{ margin: 0, fontSize: '36px', fontWeight: '800' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${cashAvailable.toLocaleString()}`}</p></div>
                  <span style={{ fontSize: '32px', opacity: 0.8 }}>💰</span>
                </div>
                <p style={{ fontSize: '13px', opacity: 0.9 }}>Tap to add income</p>
              </div>

              <div onClick={() => setActiveTab('goals')} style={{ background: darkMode ? 'linear-gradient(135deg, #3b82f6 0%, #60a5fa 100%)' : 'linear-gradient(135deg, #60a5fa 0%, #3b82f6 100%)', borderRadius: '20px', padding: '28px', color: 'white', cursor: 'pointer', boxShadow: '0 8px 24px rgba(59,130,246,0.3)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                  <div><p style={{ margin: '0 0 4px 0', fontSize: '14px', fontWeight: '600', opacity: 0.9 }}>💰 Savings</p><p style={{ margin: 0, fontSize: '36px', fontWeight: '800' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${savings.toLocaleString()}`}</p></div>
                  <span style={{ fontSize: '32px', opacity: 0.8 }}>💵</span>
                </div>
                <div style={{ background: 'rgba(255,255,255,0.2)', borderRadius: '8px', padding: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '12px' }}><span>Progress</span><span>{Math.round(savingsProgress)}%</span></div>
                  <div style={{ width: '100%', height: '6px', background: 'rgba(255,255,255,0.3)', borderRadius: '3px', overflow: 'hidden' }}><div style={{ width: `${Math.min(100, savingsProgress)}%`, height: '100%', background: 'white', borderRadius: '3px' }}></div></div>
              ...
              <div onClick={() => setActiveTab('cards')} style={{ background: debtPercentage > 50 ? 'linear-gradient(135deg, #dc2626 0%, #ef4444 100%)' : (darkMode ? 'linear-gradient(135deg, #1e293b 0%, #334155 100%)' : 'linear-gradient(135deg, #f1f5f9 0%, #e2e8f0 100%)'), borderRadius: '20px', padding: '28px', color: debtPercentage > 50 ? 'white' : (darkMode ? '#f8fafc' : '#0f172a'), cursor: 'pointer', boxShadow: debtPercentage > 50 ? '0 8px 24px rgba(239,68,68,0.3)' : (darkMode ? '0 8px 24px rgba(0,0,0,0.3)' : '0 8px 24px rgba(0,0,0,0.08)') }}>
                ...
                {creditCards.length > 0 && (
                  <div style={{ background: debtPercentage > 50 ? 'rgba(255,255,255,0.2)' : (darkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)'), borderRadius: '8px', padding: '12px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '12px' }}><span>Credit Usage</span><span>{debtPercentage.toFixed(1)}%</span></div>
                    <div style={{ width: '100%', height: '6px', background: debtPercentage > 50 ? 'rgba(255,255,255,0.3)' : (darkMode ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)'), borderRadius: '3px', overflow: 'hidden' }}><div style={{ width: `${debtPercentage}%`, height: '100%', background: debtPercentage > 50 ? '#fbbf24' : (darkMode ? '#60a5fa' : '#3b82f6'), borderRadius: '3px' }}></div></div>
              ...
              <div style={{ background: darkMode ? '#1e293b' : 'white', borderRadius: '20px', padding: '28px', boxShadow: darkMode ? '0 8px 24px rgba(0,0,0,0.3)' : '0 8px 24px rgba(0,0,0,0.08)' }}>
                ...
                <div style={{ position: 'relative', width: '200px', height: '200px', margin: '0 auto 24px' }}>
                  <svg viewBox="0 0 100 100" style={{ transform: 'rotate(-90deg)', width: '100%', height: '100%' }}>
                    ...
                    <circle cx="50" cy="50" r="40" fill="none" stroke="url(#incomeGradient)" strokeWidth="8" strokeDasharray={`${Math.min(100, incomeProgress) * 2.51} 251`} strokeLinecap="round" />
                    <defs><linearGradient id="incomeGradient" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#14b8a6" /><stop offset="100%" stopColor="#8b5cf6" /></linearGradient></defs>
                  </svg>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                  <div style={{ background: darkMode ? '#0f172a' : '#f8fafc', padding: '16px', borderRadius: '12px', textAlign: 'center' }}><p style={{ margin: '0 0 4px 0', fontSize: '12px', color: darkMode ? '#94a3b8' : '#647480' }}>Earned</p><p style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: '#14b8a6' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${monthlyIncome.toLocaleString()}`}</p></div>
                  <div style={{ background: darkMode ? '#0f172a' : '#f8fafc', padding: '16px', borderRadius: '12px', textAlign: 'center' }}><p style={{ margin: '0 0 4px 0', fontSize: '12px', color: darkMode ? '#94a3b8' : '#647480' }}>Goal</p><p style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: '#8b5cf6' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${monthlyIncomeGoal.toLocaleString()}`}</p></div>
                </div>
              </div>
                  ...
                  <div style={{ display: 'grid', gap: '16px' }}>
                    {monthlyGoals.slice(0, 3).map((goal) => (
                      <div key={goal.id} style={{ background: darkMode ? '#0f172a' : '#f8fafc', padding: '16px', borderRadius: '12px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}><p style={{ margin: 0, fontSize: '14px', fontWeight: '600', color: darkMode ? '#f8fafc' : '#0f172a' }}>{goal.name}</p><p style={{ margin: 0, fontSize: '14px', fontWeight: '700', color: goal.color }}>{Math.round((goal.current / goal.target) * 100)}%</p></div>
                        <div style={{ width: '100%', height: '8px', background: darkMode ? '#334155' : '#e2e8f0', borderRadius: '4px', overflow: 'hidden' }}><div style={{ width: `${Math.min(100, (goal.current / goal.target) * 100)}%`, height: '100%', background: goal.color, borderRadius: '4px' }}></div></div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px', fontSize: '12px' }}><span style={{ color: darkMode ? '#94a3b8' : '#647480' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${goal.current.toLocaleString()}`}</span><span style={{ color: darkMode ? '#94a3b8' : '#647480' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${goal.target.toLocaleString()}`}</span></div>
                      </div>
                    ))}
                    {monthlyGoals.length > 3 && <button onClick={() => setActiveTab('goals')} style={{ padding: '12px', background: 'transparent', border: `2px dashed ${darkMode ? '#334155' : '#e2e8f0'}`, borderRadius: '10px', color: darkMode ? '#94a3b8' : '#647480', cursor: 'pointer', fontWeight: '600' }}>+{monthlyGoals.length - 3} more goals</button>}
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ background: darkMode ? '#1e293b' : 'white', borderRadius: '20px', padding: '28px', boxShadow: darkMode ? '0 8px 24px rgba(0,0,0,0.3)' : '0 8px 24px rgba(0,0,0,0.08)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                <button onClick={() => setActiveTab('income')} style={{ padding: '20px', background: 'linear-gradient(135deg, #14b8a6 0%, #10b981 100%)', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: '700', fontSize: '14px', textAlign: 'left' }}><span style={{ fontSize: '24px', display: 'block', marginBottom: '8px' }}>💵</span>Add Income<p style={{ margin: '4px 0 0 0', fontSize: '12px', fontWeight: '400', opacity: 0.9 }}>Increase cash balance</p></button>
                <button onClick={() => setActiveTab('expenses')} style={{ padding: '20px', background: 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: '700', fontSize: '14px', textAlign: 'left' }}><span style={{ fontSize: '24px', display: 'block', marginBottom: '8px' }}>📋</span>Add Expense<p style={{ margin: '4px 0 0 0', fontSize: '12px', fontWeight: '400', opacity: 0.9 }}>Track spending</p></button>
                <button onClick={() => setActiveTab('cards')} style={{ padding: '20px', background: 'linear-gradient(135deg, #8b5cf6 0%, #60a5fa 100%)', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: '700', fontSize: '14px', textAlign: 'left' }}><span style={{ fontSize: '24px', display: 'block', marginBottom: '8px' }}>💳</span>Manage Cards<p style={{ margin: '4px 0 0 0', fontSize: '12px', fontWeight: '400', opacity: 0.9 }}>View & pay debts</p></button>
                <button onClick={() => setActiveTab('goals')} style={{ padding: '20px', background: 'linear-gradient(135deg, #f59e0b 0%, #f97316 100%)', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: '700', fontSize: '14px', textAlign: 'left' }}><span style={{ fontSize: '24px', display: 'block', marginBottom: '8px' }}>🎯</span>Set Goals<p style={{ margin: '4px 0 0 0', fontSize: '12px', fontWeight: '400', opacity: 0.9 }}>Plan your savings</p></button>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'cards' && (
          <div style={{ background: darkMode ? '#1e293b' : 'white', borderRadius: '20px', padding: '28px', boxShadow: darkMode ? '0 8px 24px rgba(0,0,0,0.3)' : '0 8px 24px rgba(0,0,0,0.08)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px' }}>
              <div><h2 style={{ margin: '0 0 8px 0', fontSize: '24px', fontWeight: '800', color: darkMode ? '#f8fafc' : '#0f172a' }}>💳 Credit Cards</h2><p style={{ margin: 0, fontSize: '14px', color: darkMode ? '#94a3b8' : '#647480' }}>Manage your cards and track debts</p></div>
                ...
                {creditCards.map((card) => (
                  <div key={card.id} style={{ background: darkMode ? '#0f172a' : '#f8fafc', padding: '24px', borderRadius: '16px', border: `2px solid ${darkMode ? '#334155' : '#e2e8f0'}` }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '20px' }}>
                      <div style={{ background: darkMode ? '#1e293b' : 'white', padding: '16px', borderRadius: '12px', textAlign: 'center' }}><p style={{ margin: '0 0 4px 0', fontSize: '12px', color: darkMode ? '#94a3b8' : '#647480' }}>Limit Quota</p><p style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: darkMode ? '#f8fafc' : '#0f172a' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${card.limit.toLocaleString()}`}</p></div>
                      <div style={{ background: darkMode ? '#1e293b' : 'white', padding: '16px', borderRadius: '12px', textAlign: 'center' }}><p style={{ margin: '0 0 4px 0', fontSize: '12px', color: darkMode ? '#94a3b8' : '#647480' }}>Available</p><p style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: '#14b8a6' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${card.available.toLocaleString()}`}</p></div>
                      <div style={{ background: darkMode ? '#1e293b' : 'white', padding: '16px', borderRadius: '12px', textAlign: 'center' }}><p style={{ margin: '0 0 4px 0', fontSize: '12px', color: darkMode ? '#94a3b8' : '#647480' }}>Debt to Pay</p><p style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: '#ef4444' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${card.balance.toLocaleString()}`}</p></div>
                    </div>
        ...
        {activeTab === 'goals' && (
          <div style={{ background: darkMode ? '#1e293b' : 'white', borderRadius: '20px', padding: '28px', boxShadow: darkMode ? '0 8px 24px rgba(0,0,0,0.3)' : '0 8px 24px rgba(0,0,0,0.08)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px' }}>
              <div><h2 style={{ margin: '0 0 8px 0', fontSize: '24px', fontWeight: '800', color: darkMode ? '#f8fafc' : '#0f172a' }}>🎯 Monthly Goals</h2><p style={{ margin: 0, fontSize: '14px', color: darkMode ? '#94a3b8' : '#647480' }}>Set and track your financial goals</p></div>
                ...
                {monthlyGoals.map((goal) => (
                  <div key={goal.id} style={{ background: darkMode ? '#0f172a' : '#f8fafc', padding: '24px', borderRadius: '16px', border: `2px solid ${darkMode ? '#334155' : '#e2e8f0'}` }}>
                    ...
                    <div style={{ marginBottom: '20px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '14px' }}><span style={{ color: darkMode ? '#94a3b8' : '#647480' }}>Progress</span><span style={{ fontWeight: '700', color: goal.color }}>{Math.round((goal.current / goal.target) * 100)}%</span></div>
                      <div style={{ width: '100%', height: '12px', background: darkMode ? '#334155' : '#e2e8f0', borderRadius: '6px', overflow: 'hidden' }}><div style={{ width: `${Math.min(100, (goal.current / goal.target) * 100)}%`, height: '100%', background: goal.color, borderRadius: '6px', transition: 'width 0.5s ease' }}></div></div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '12px', fontSize: '14px' }}><span style={{ color: darkMode ? '#94a3b8' : '#647480' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${goal.current.toLocaleString()}`}</span><span style={{ color: darkMode ? '#94a3b8' : '#647480' }}>{hideNumbers ? CONFIG.currency + '••••' : `${CONFIG.currency}${goal.target.toLocaleString()}`}</span></div>
                    </div>
                  ...
                  <div key={expense.id} style={{ background: darkMode ? '#0f172a' : '#f8fafc', padding: '20px', borderRadius: '12px', border: `2px solid ${darkMode ? '#334155' : '#e2e8f0'}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div><p style={{ margin: 0, fontSize: '16px', fontWeight: '700', color: darkMode ? '#f8fafc' : '#0f172a' }}>{expense.description || expense.category}</p><p style={{ margin: '4px 0 0 0', fontSize: '13px', color: darkMode ? '#94a3b8' : '#647480' }}>{expense.cardId === 'cash' ? '💵 Cash' : creditCards.find(c => c.id === parseInt(expense.cardId))?.name} • {expense.date}</p></div>
                      </div>
                ...
                {dailyIncomes.map((income, index) => (
                  <div key={income.id} style={{ background: darkMode ? '#0f172a' : '#f8fafc', padding: '20px', borderRadius: '12px', border: `2px solid ${darkMode ? '#334155' : '#e2e8f0'}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <span style={{ fontSize: '28px' }}>💵</span>
                        <div><p style={{ margin: 0, fontSize: '16px', fontWeight: '700', color: darkMode ? '#f8fafc' : '#0f172a' }}>Daily Income</p><p style={{ margin: '4px 0 0 0', fontSize: '13px', color: darkMode ? '#94a3b8' : '#647480' }}>{income.date}</p></div>
                      </div>
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        ...
                        {index === 0 && (<><button onClick={handleEditIncome} style={{ padding: '8px 12px', background: '#f59e0b', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>✏️</button><button onClick={handleDeleteIncome} style={{ padding: '8px 12px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>🗑️</button></>)}
                      </div>
                    </div>
                  </div>
                ))}
